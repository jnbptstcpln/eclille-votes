...
class LastCampaignView(AbstractBdxView):
    ...
    def get(self, req):
        ...
        campaigns = Campaign.objects.filter(starts_on__lt=datetime.datetime(year=current_school_year(), month=9, day=1)).order_by("starts_on")
        ...
        campaigns_year_sorted = collections.OrderedDict(sorted(campaigns_year.items(), reverse=True))
...
class CampaignVoteBoxView(AbstractBdxView):
    def get(self, req, campaign_pk):
        ...
        return render(
            ...
            {'election': f"{campaign.type.upper()} {campaign.starts_on.year}"},
